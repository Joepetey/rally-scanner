...
class TestScanAll:
    ...
    def _mock_pool():
        ...
        return patch("pipeline.scanner.parallel.ProcessPoolExecutor", return_value=mock_pool)

    @patch("pipeline.scanner.parallel.fetch_quotes")
    @patch("pipeline.scanner.parallel.fetch_daily_batch")
    @patch("pipeline.scanner.parallel.fetch_vix_safe")
    @patch("pipeline.scanner.load_manifest")
    @patch("pipeline.scanner.parallel.load_model")
