...
Usage:
    python backtest_universe.py                           # full universe (~512 assets, ~2-3 hours)
...
def train_single(ticker: str, vix_data: pd.Series | None = None) -> pd.DataFrame | None:
    ...
    try:
        ...
        # Auto-calibrate thresholds from historical vol
...
def cache_all_predictions(tickers: list[str], resume: bool = False) -> dict[str, pd.DataFrame]:
    ...
    for i, ticker in enumerate(remaining, 1):
        ...
        preds = train_single(ticker, vix_data=vix_data)
        elapsed = time.time() - t0

        if preds is not None and len(preds) > 0:
            cached[ticker] = preds
            n_folds = len(preds.index.year.unique())
            print(f"{len(preds)} OOS bars ({elapsed:.1f}s)")
            success += 1
        else:
            reason = "insufficient data" if preds is None else "no predictions"
            print(f"SKIP ({reason}, {elapsed:.1f}s)")
            failed += 1

        # Save checkpoint every 25 assets
        if i % 25 == 0:
...
def plot_results(results: list[dict], per_asset: pd.DataFrame) -> None:
    ...
    plt.tight_layout()
    plt.savefig("universe_frontier.png", dpi=150)
    plt.close()
    print("  Saved: universe_frontier.png")
    ...
    plt.tight_layout()
    plt.savefig("universe_equity_curves.png", dpi=150)
    plt.close()
    print("  Saved: universe_equity_curves.png")
    ...
    if not per_asset.empty and len(per_asset) > 5:
        ...
        plt.tight_layout()
        plt.savefig("universe_asset_distributions.png", dpi=150)
        plt.close()
        print("  Saved: universe_asset_distributions.png")
    ...
    if not per_asset.empty and len(per_asset) > 20:
        ...
        plt.tight_layout()
        plt.savefig("universe_top_bottom_assets.png", dpi=150)
        plt.close()
        print("  Saved: universe_top_bottom_assets.png")
