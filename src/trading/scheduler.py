...
class TradingScheduler:
    ...
    async def run_daily_scan(
        ...
        positions, closed, signals, positions_for_embed = await self._prepare_scan_signals(
            results, manifest
        )
    ...
    async def _prepare_scan_signals(
        self, results: list[dict], manifest: dict
    ) -> tuple[dict, list[dict], list[dict], list[dict]]:
        """Sync positions, detect exits, apply cooldown filter to scan results.

        Returns (positions, closed, signals, positions_for_embed).
        """
        all_signals = [r for r in results if r.get("signal")]
        ...
        return positions, closed, signals, positions_for_embed
